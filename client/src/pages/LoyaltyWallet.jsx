...
export default function LoyaltyWallet() {
  ...
  const rewardVouchers = [
    ...
    {
      id: 2,
      title: "₹100 Ticket Discount Voucher",
      cost: 200,
      code: "CINE100-POINT",
  ...
  const handleRedeem = async (voucher) => {
    ...
    setIsRedeeming(true);
    try {
      // Simulate API Call: await API.post("/api/rewards/redeem", { voucherId: voucher.id });
  ...
  return (
    <div className="bg-[#07070B] min-h-screen text-white p-4 md:p-10 relative selection:bg-amber-500 selection:text-black">
      {/* Background glow */}
        ...
        {redeemedCode && (
          ...
          >
            <div
              ...
              className="bg-[#0F0F17] border border-amber-500/40 p-8 rounded-3xl text-center space-y-6 max-w-sm w-full shadow-2xl relative"
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-400">
                  Use this voucher promo code at checkout
                </p>
              </div>

              <div className="bg-gray-950 border border-amber-500/30 p-4 rounded-2xl flex items-center justify-between font-mono text-amber-400 font-black text-base tracking-wider">
                <span>{redeemedCode.code}</span>
                <button
                  onClick={() => copyToClipboard(redeemedCode.code)}
                  className="text-xs bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 px-2.5 py-1 rounded-lg transition font-sans"
                >
                  {copied ? "Copied!" : "Copy"}
                </button>
              </div>
