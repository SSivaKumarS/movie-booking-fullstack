    ...
    <div className="min-h-screen bg-[#08080E] text-white pb-36 font-sans relative overflow-x-hidden selection:bg-red-600 selection:text-white">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => {
                ...
                alert("Group Booking Link copied!");
              }}
              className="bg-red-950/80 hover:bg-red-900 border border-red-800 text-red-300 font-bold text-xs px-3 py-1.5 rounded-xl transition"
            >
              Share Seats Link
            </button>
            ...
            <button
              onClick={() => {
                const query = encodeURIComponent(`${show?.theatre || "Cinema"}`);
                window.open(`https://www.google.com/maps/search/?api=1&query=${query}`, "_blank");
              }}
              className="bg-gray-900 hover:bg-gray-800 border border-gray-800 text-gray-300 font-bold text-xs px-3 py-1.5 rounded-xl transition"
            >
              GPS Directions
            </button>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-[#0E0E16]/80 border border-gray-800/80 rounded-3xl p-6 sm:p-10 shadow-2xl backdrop-blur-2xl space-y-10">
          ...
          {/* SPECIAL COMPANION / ACCESSIBILITY (CS) BLOCK */}
          <div className="p-4 bg-gray-950/60 border border-gray-800/90 rounded-2xl space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-gray-800/60">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black uppercase tracking-wider text-blue-400">
                  CS Section — Extra Space & Companion Access
                </span>
              </div>
              <span className="text-xs font-semibold text-gray-400">₹200</span>
            </div>

            <div className="flex items-center justify-center gap-3">
              <span className="w-5 text-center text-xs font-black text-gray-500 select-none">CS</span>
              <div className="flex items-center gap-2">
                {["CS1", "CS2", "CS3", "CS4"].map((csNumber) => {
                  const isSelected = selectedSeats.includes(csNumber);
                  return (
                    <button
                      key={csNumber}
                      onClick={() =>
                        setSelectedSeats((prev) =>
                          prev.includes(csNumber)
                            ? prev.filter((s) => s !== csNumber)
                            : [...prev, csNumber]
                        )
                      }
                      onMouseEnter={() =>
                        setHoveredSeat({
                          seatNumber: csNumber,
                          category: "CS Extra Space",
                          price: 200,
                          isBooked: false,
                        })
                      }
                      onMouseLeave={() => setHoveredSeat(null)}
                      className={`w-9 h-9 rounded-xl text-[11px] font-extrabold transition-all duration-150 flex items-center justify-center border ${
                        isSelected
                          ? "bg-blue-600 border-white text-white shadow-lg shadow-blue-600/50 scale-110"
                          : "bg-gray-900 border-blue-950 text-blue-300 hover:border-blue-500 hover:text-white"
                      }`}
                    >
                      {csNumber}
                    </button>
      ...
      <div className="max-w-xl mx-auto px-4 mt-8 space-y-4">
        {hoveredSeat && (
          <div className="bg-red-950/50 border border-red-800/80 rounded-2xl p-3 text-center text-xs text-white">
        ...
        <div className="flex items-center justify-around bg-[#0D0D14]/90 border border-gray-800/80 rounded-2xl p-4 text-xs text-gray-400 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-lg bg-blue-900/80 border border-blue-500" />
            <span>CS Block</span>
          </div>
          <div className="flex items-center gap-2">
